...
def reduce(tensor: Tensor, pattern: str, reduction: Reduction, **axes_lengths: int) -> Tensor:
    """
    meinops.reduce provides combination of reordering and reduction using reader-friendly notation.
...
def rearrange(tensor: Union[Tensor, List[Tensor]], pattern: str, **axes_lengths) -> Tensor:
    """
    meinops.rearrange is a reader-friendly smart element reordering for multidimensional tensors.
    ...
    Find more examples in meinops tutorial.
...
def repeat(tensor: Tensor, pattern: str, **axes_lengths) -> Tensor:
    """
    meinops.repeat allows reordering elements and repeating them in arbitrary combinations.
...
def einsum(*tensors_and_pattern: Union[Tensor, str]) -> Tensor:
    """
    meinops.einsum calls einsum operations with meinops-style named
    ...
    if len(tensors_and_pattern) <= 1:
        raise ValueError(
            "`meinops.einsum` takes at minimum two arguments: the tensors (at least one),"
            " followed by the pattern."
        )
    pattern = tensors_and_pattern[-1]
    if not isinstance(pattern, str):
        raise ValueError(
            "The last argument passed to `meinops.einsum` must be a string,"
